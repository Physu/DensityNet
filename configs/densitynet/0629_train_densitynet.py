model = dict(
    ...
    backbone=dict(
        ...
        fps_mods=('D-FPS', 'MS3', ('F-FPS', 'D-FPS')),
        ...
        sa_cfg=dict(
            ...
            normalize_xyz=False,
            use_origin_sa=True,  # use original version or auther designed  # 如果使用原版sa，use_origin_sa 置为 True, 和return_triple 和 return_group_xyz 需要置为False
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
optimizer = dict(type='AdamW', lr=0.02, weight_decay=0)
...
# :todo 0629
# 修改mask sample，以及backbone中SA，只在第三个SA使用se 模块
