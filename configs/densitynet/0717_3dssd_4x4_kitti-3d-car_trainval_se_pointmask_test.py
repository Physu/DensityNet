model = dict(
    ...
    backbone=dict(
        ...
        sa_channels=(((16, 16, 32), (16, 16, 32), (32, 32, 64)),
                     ...
                     ((128, 128, 256), (128, 192, 256), (128, 256, 256))),  # 定义的是sa中 mlp_channels维度，其实就是QueryAndGroup中最后的维度[B,]
        aggregation_channels=(64, 128, 256),  # 定义在pointnet2中，处理pointnet中SA返回的数据
        ...
        #     pool_mod='max',
        #     use_xyz=False,  # True: return xyz+feat False:return feat, there is no other option, so set "False" first, but "return_triple" we get same as set "True"
        #     normalize_xyz=False,
        #     use_origin_sa=True,  # use original version or auther designed
        #     return_grouped_xyz=True,  # whether return xyz and feat separately in QueryAndGroup
        #     return_triple=True  # whether return xyz + feat
        # )),
        sa_cfg=dict(
            type='PointSAModuleMSGMaskV1',
            pool_mod='max',
            use_xyz=True,  # True: return xyz+feat False:return feat, there is no other option, so set "False" first, but "return_triple" we get same as set "True"
            normalize_xyz=False,
            use_origin_sa=False,  # use original version or auther designed
            return_grouped_xyz=False,  # whether return xyz and feat separately in QueryAndGroup
            return_triple=False  # whether return xyz + feat
        )),
    bbox_head=dict(
        ...
        vote_loss=dict(
            type='SmoothL1Loss', reduction='sum', loss_weight=1.0),
        # mask_loss=dict(  # if set valid, change densitymasknet_3d_head L84 uncomment
        #     type='CrossEntropyLoss', reduction='sum', loss_weight=1.0, use_sigmoid=True),
        num_classes=1,
        bbox_coder=dict(
            type='AnchorFreeBBoxCoder', num_dir_bins=12, with_rot=True)),
    train_cfg=dict(
        sample_mod='spec', pos_distance_thr=10.0, expand_dims_length=0.05),  # 注意这里的定义
    test_cfg=dict(
...
data = dict(
    ...
    test=dict(
        ...
        split='testing',  # 这里也要记得修改，否则寻找文件的位置会在training下
        pts_prefix='velodyne_reduced',
        pipeline=[
            dict(
                type='LoadPointsFromFile',
...
log_level = 'INFO'
work_dir = 'newback/0624_3dssd_official'
load_from = None
resume_from = None
workflow = [('train', 1)]
lr = 0.001  # single gpu, batchsize=8
optimizer = dict(type='AdamW', lr=0.001, weight_decay=0)
optimizer_config = dict(grad_clip=dict(max_norm=35, norm_type=2))
lr_config = dict(policy='step', warmup=None, step=[90, 120])
