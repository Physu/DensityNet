...
class DeltaXYZWLHRBBoxCoder(BaseBBoxCoder):
    ...
    def encode(src_boxes, dst_boxes):
        ...
        za = za + ha / 2  # 相当于将高度给提高到3d box 的上表面
        zg = zg + hg / 2
    ...
    def decode(anchors, deltas):
        ...
        if box_ndim > 7:
            ...
            xa, ya, za, wa, la, ha, ra = torch.split(anchors, 1, dim=-1)  # anchor
            xt, yt, zt, wt, lt, ht, rt = torch.split(deltas, 1, dim=-1)

        za = za + ha / 2  # 这里是不是应该za-ha/2
        diagonal = torch.sqrt(la**2 + wa**2)
        xg = xt * diagonal + xa
        yg = yt * diagonal + ya
        zg = zt * ha + za
        ...
        return torch.cat([xg, yg, zg, wg, lg, hg, rg, *cgs], dim=-1)  # 相当于转换成gt格式
