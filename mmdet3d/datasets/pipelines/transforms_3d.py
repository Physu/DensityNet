...
class ObjectSample(object):
    ...
    def __call__(self, input_dict):
        ...
        if self.sample_2d:
            ...
            sampled_dict = self.db_sampler.sample_all(
                gt_bboxes_3d.tensor.numpy(), gt_labels_3d, img=None)  # 经过碰撞测试后，采样的得到不同类别的gt_instances, 来做数据增广
        ...
        if sampled_dict is not None:
            ...
            gt_labels_3d = np.concatenate([gt_labels_3d, sampled_gt_labels],
                                          axis=0)
            gt_bboxes_3d = gt_bboxes_3d.new_box(  # 构建出来和gt一样的 LiDARInstance3DBoxes
                np.concatenate(
                    [gt_bboxes_3d.tensor.numpy(), sampled_gt_bboxes_3d]))

            points = self.remove_points_in_boxes(points, sampled_gt_bboxes_3d)  # 这一步将采样得到的sp_gt所处位置的点，都给去除
