...
from mmdet3d.ops import GroupAll, PAConv, Points_Sampler, QueryAndGroup, gather_points, QueryAndGroupV2
...
class PointSAModuleMSGMaskV1(nn.Module):
    """Point set abstraction module with multi-scale grouping used in
    Pointnets.
     :todo 重新设计的SA module, 一共3个SA包含三个不同尺度的radius，只在最后一个SA模块中采用SENet形式来设计。这个方案减小了模型，2080ti可以
     batchsize 4 训练
    ...
    def __init__(self,
        ...
        for i in range(len(radii)):
            ...
            if self.use_origin_sa and sa_index == 2:
        ...
        if self.use_origin_sa:
            ...
            for i in range(len(self.groupers)):
                ...
                if sa_index != 2:
                    ...
                    new_features = new_features[2]  # use QueryAndGroup2, so added to decode the new_features
                    ...
                    new_features_list.append(new_features)  # above is origin SA
            ...
            for i in range(len(self.groupers)):
                ...
                # 返回的feat + xyz：4，67，131 相当于分解过后，和最后混在一起的
                # (B, mlp[-1], num_point, nsample)
...
@SA_MODULES.register_module()
class PointSAModuleMaskV1(PointSAModuleMSGMaskV1):
