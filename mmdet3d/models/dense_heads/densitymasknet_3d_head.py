...
class DensityMaskNet3DHead(VoteHead):
    ...
    def __init__(self,
                 ...
                 vote_loss=None,
                 mask_loss=None):
        super(DensityMaskNet3DHead, self).__init__(
            num_classes,
            bbox_coder,
            train_cfg=train_cfg,
            test_cfg=test_cfg,
    ...
    def _get_reg_out_channels(self):
        ...
        return 3 + 3 + self.num_dir_bins * 2  # why multipy 2?
    ...
    def _extract_input(self, feat_dict):
        ...
        point_wise_mask = feat_dict['sa_point_wise_mask']
        mask_preds = feat_dict['sa_mask_preds']  # 用于计算 mask loss 所用的信息

        return seed_points, seed_features, seed_indices, point_wise_mask, mask_preds
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        seed_points, seed_features, seed_indices, point_wise_mask, mask_preds = self._extract_input(
            feat_dict)
        ...
        '''
         ...
         原文链接：https: // blog.csdn.net / wqwqqwqw1231 / article / details / 101283243、
         ...
         '''
        # ：todo 从seeds 得到votes，votes并不在gt instance表面，属于同一个instance的votes聚集的更加紧密
        ...
        vote_aggregation_ret = self.vote_aggregation(**aggregation_inputs)  # 这里调用了sa module
        if len(vote_aggregation_ret) == 3:  # 调用point_sa_module
            aggregated_points, features, aggregated_indices = vote_aggregation_ret
        if len(vote_aggregation_ret) == 4:  # 调用point_sa_module_mask
            aggregated_points, features, aggregated_indices, _ = vote_aggregation_ret  # 这个地方特别明显 sa module

        results['aggregated_points'] = aggregated_points
        results['aggregated_features'] = features
        results['aggregated_indices'] = aggregated_indices
        results['point_wise_mask'] = point_wise_mask
    ...
    def loss(self,
        ...
        if bbox_preds['mask_preds'][1] is None:
            mask_loss = torch.tensor(0.0)
        else:
            B = len(point_wise_mask)
            mask_preds = bbox_preds['mask_preds'][1].transpose(1, 2)   # Tensor [4,4096,2]->[4,2,4096] 将class调整到中间，为cross entropy 计算做好准备
            mask_preds = mask_preds.squeeze(dim=1)

            point_wise_mask_2 = bbox_preds['point_wise_mask'][1]  # 之所以用索引1，是因为这个是对于4096个mask
            point_wise_mask_2_reshape = torch.cat(point_wise_mask_2, dim=0).view(B, -1).long()  # Tensor[B, 4096], 不考虑0的贡献
            # numof1 = torch.nonzero(point_wise_mask_2_reshape)
            # sumof1 = point_wise_mask_2_reshape.sum()
            # 下面这种mask_loss_weights相当于只关注那个true的点，那些错误分类的情况就不予考虑了，梯度并不回传
            # mask_loss_weights = point_wise_mask_2_reshape / (point_wise_mask_2_reshape.sum() + 1e-6)
            # 为了解决这个问题将权重weight 改写为下列形式，对于负例也纳入考量
            mask_length = len(point_wise_mask_2_reshape[0])
            total = B * len(point_wise_mask_2_reshape[0])
            ones = torch.ones([B, len(point_wise_mask_2_reshape[0])])
            mask_loss_weights_include_negative = torch.ones([B, len(point_wise_mask_2_reshape[0])]) / (B * len(point_wise_mask_2_reshape[0]) + 1e-6)
            # point-wise mask loss
            # with open('points_to_show/gt_bboxes_3d_np_backup_corners.npy', 'ab+') as f:
            #     np.save(f, gt_bboxes_3d_np_backup_corners)
            mask_loss = self.mask_loss(
                mask_preds, point_wise_mask_2_reshape,
                weight=mask_loss_weights_include_negative.cuda()
            )
        # mask_loss = torch.tensor(0.0, requires_grad=False).cuda()

        # calculate centerness loss, CrossEntropyLoss
        centerness_loss = self.objectness_loss(
            bbox_preds['obj_scores'].transpose(1, 2),
            centerness_targets,
            weight=centerness_weights)

        # centerness loss 和 center loss有什么区别？
    ...
    def get_targets(self,
        ...
        seed_points = [
            bbox_preds['seed_points'][i, :self.num_candidates].detach()  # only use self.numcandidates points
            for i in range(len(gt_labels_3d))
        ]
    ...
    def get_targets_single(self,
        ...
        valid_gt = gt_labels_3d != -1  # get location where != -1
        ...
        if valid_gt.sum() == 0:
            ...
            positive_mask = points.new_zeros(  # positive 用 0 来表示吗？
        ...
        dist_mask = dist < self.train_cfg.pos_distance_thr  # the thr is 10.0
        ...
        vote_mask, vote_assignment = self._assign_targets_by_points_inside(
            enlarged_gt_bboxes_3d, seed_points)  # expand 0.05,扩大一点
