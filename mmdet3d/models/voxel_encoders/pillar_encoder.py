...
class PillarFeatureNet(nn.Module):
    ...
    def __init__(self,
        ...
        if with_voxel_center:
            in_channels += 2
        if with_distance:  # 这里false，故不包括，所以 Linear层 in_features=4+3+2=9
            in_channels += 1
        ...
        self.x_offset = self.vx / 2 + point_cloud_range[0]  # 用来作甚，暂不明朗
    ...
    def forward(self, features, num_points, coors):
        ...
        if self._with_cluster_center:  # 默认 True
            points_mean = features[:, :, :3].sum(
                dim=1, keepdim=True) / num_points.type_as(features).view(
                    -1, 1, 1)  # 这一步就是voxel中点xyz三个维度取均值操作了
            f_cluster = features[:, :, :3] - points_mean
            features_ls.append(f_cluster)  # 此时，features_ls 包含 原始voxel信息，和减去均值后的 voxels信息（其实就是将位置做了个修正）
        ...
        if self._with_voxel_center:
            if not self.legacy:
                ...
                f_center = features[:, :, :2]  # 获取voxel中32个点的xy 值
                f_center[:, :, 0] = f_center[:, :, 0] - (
                    coors[:, 3].type_as(features).unsqueeze(1) * self.vx +
                    self.x_offset)  # 这里之所以写成这样，主要是coors在voxelization的时候有转换 见mmdet3d/ops/voxel/voxelize.py L98-100，可以看到之间的关系
                f_center[:, :, 1] = f_center[:, :, 1] - (
                    coors[:, 2].type_as(features).unsqueeze(1) * self.vy +
                    self.y_offset)  # f_center 表示的是获取voxel中点 相对于自己所在voxel中心点xy平面上的偏移
            features_ls.append(f_center) # # 此时，1.features_ls 包含 原始voxel信息，2.和减去均值后的 voxels信息f_cluster（其实就是将位置做了个修,3. 点相对于自己所在voxel的偏移信息

        if self._with_distance:
            points_dist = torch.norm(features[:, :, :3], 2, 2, keepdim=True)
            features_ls.append(points_dist)  # 计算每个点距离lidar scanner的距离

        # Combine together feature decorations
        features = torch.cat(features_ls, dim=-1)  # 将最后一维concatenate到一起
        # The feature decorations were calculated without regard to whether
        # pillar was empty. Need to ensure that
        # empty pillars remain set to zeros.
        voxel_count = features.shape[1]
        mask = get_paddings_indicator(num_points, voxel_count, axis=0)  # Mask indicates which points are valid inside a voxel. 说白了比如一个batch中，共有N个非空voxel，那就要知道这个voxel中32个点，那几个非空，这个mask，就是用来做这个
        mask = torch.unsqueeze(mask, -1).type_as(features)
        features *= mask  # 相当于把那些没有意义的xyz,mean xyz,relative center xyz 全部直接置为0.，但是tensor shape 不改变

        for pfn in self.pfn_layers:
            features = pfn(features, num_points)  # [N,1,64] N 是同一个batch中所有不为空的voxel数目，相当于linear,batchnorm 之后，再dim=1维度上，32个中取得最大值
